...
if [ -z "$(tmux show-option -gqv @open-file-nvim-key)" ]; then
    tmux bind 'F' run-shell "$CURRENT_DIR/open-file-nvim.sh"
else
...
if [ -z "$(tmux show-option -gqv @open-file-nvim-all-key)" ]; then
    tmux bind 'H' run-shell "$CURRENT_DIR/open-file-nvim.sh --selected-pane-history"
else
...
if [ -z "$(tmux show-option -gqv @open-file-nvim-all-history-key)" ]; then
    tmux bind 'G' run-shell "$CURRENT_DIR/open-file-nvim.sh --all-pane-history"
else
